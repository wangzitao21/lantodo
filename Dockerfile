...
LABEL org.opencontainers.image.title="LanTodo NAS" org.opencontainers.image.version="1.0" org.opencontainers.image.licenses="GPL-3.0-only"
WORKDIR /app
COPY --from=build /out/ ./
...
USER app
ENV LANTODO_DATA=/data LANTODO_BACKUPS=/backups LANTODO_NAME="LanTodo NAS"
VOLUME ["/data", "/backups"]
EXPOSE 42851/tcp
