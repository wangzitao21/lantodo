...
新增本机元数据键 `replica-settings-v1.json`：保存 NAS 启用状态、局域网发现/直连开关及固定节点地址（证书 ID + 地址）。默认仅启用原局域网同步。此配置不随版本同步或数据 ZIP 导出；整库位置迁移会保留。Linux NAS 额外使用 `LanTodo.sqlite.lock` 实现跨进程排他租约，Windows 和 Android 的原有目录行为不变。
