...
public sealed partial class PeerNode : IAsyncDisposable
{
    ...
    public string Status { get; private set; } = "当前仅本机使用";
    public string? LastError { get; private set; }
    public DateTimeOffset? LastSync { get; private set; }
    public event Action? Changed;
    public int Port => ((IPEndPoint?)listener?.LocalEndpoint)?.Port ?? 0;
    public FoundPeer[] Nearby => found.Values.Where(p => DateTimeOffset.UtcNow - p.Seen < TimeSpan.FromSeconds(100)).ToArray();
    ...
    private void SetStatus(string value, string? error = null)
    {
        lock (statusGate)
        {
            Status = store.List().Any(t => t.Conflict) ? "存在冲突，请选择最终版本 · " + value : value;
            LastError = error;
        }
        Changed?.Invoke();
    }
    ...
    private async Task<string?> SyncCoreAsync(IPEndPoint endpoint, string peerId, CancellationToken token)
    {
        ...
        {
            var page = await Exchange(new("inventory", Offset: offset));
        ...
        foreach (var batch in remoteIds.Where(id => !localIds.Contains(id)).Chunk(8))
        {
        ...
        foreach (var batch in store.Export().Where(r => !remoteSet.Contains(r.Id)).Chunk(8))
        {
        ...
        using (var tls = new SslStream(tcp.GetStream(), false, (_, cert, _, _) => cert is not null))
        {
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    if (!identity.IsTrusted(peerId)) throw new UnauthorizedAccessException("此设备未授权或授权已取消。");
                    Packet reply;
                    switch (request.Kind)
                    {
                        case "inventory":
                            ...
                            reply = new("inventory", Ids: snapshot.Skip(request.Offset).Take(256).Select(r => r.Id).ToArray(), Done: request.Offset + 256 >= snapshot.Length);
                            break;
                        case "get":
